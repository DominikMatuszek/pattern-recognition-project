...
def main():
    model = UNet()
    model.load_state_dict(torch.load("m3", map_location=torch.device('cpu')))
    model.eval()
    
    # Load image from images/test/1.jpg
    image = torchvision.io.read_image("test_masked.png", mode=torchvision.io.image.ImageReadMode.RGB)
    # Resize to 128x128
