...
class SaliencyDropout(torch.nn.Module):
    ...
    def set_mask(self, mask: torch.Tensor):
        ...
        indices = torch.cat(
            [
                torch.zeros(indices.shape[0], 1, dtype=torch.long, device=self.device),
                indices,
            ],
            dim=1,
        )
...
class ProgrammableSampler(torch.nn.Module):
    def __init__(
        self,
        model,
        dropout_rate=0.5,
        device="cpu",
        query_model_override=None,
        drop_name="patch_drop",
        saliency_fn=None,
    ):
        super(ProgrammableSampler, self).__init__()

        # If model is a string, load the model from timm
        if isinstance(model, str):
            print("Initializing model from timm")
            modelname = model
            query_model = timm.models.create_model(modelname, pretrained=True).to(
                device
            )
            ...
            if query_model_override is not None:
                ...
                query_model = timm.models.create_model(
                    query_model_override, pretrained=True
                ).to(device)

        elif isinstance(model, torch.nn.Module):
            raise Exception(
                "Model must be a string or function returning model; not a torch.nn.Module. Because we need two of them."
            )
        elif callable(model):
        ...
        if not saliency_fn:
            self.rollout = VITAttentionRollout(
                query_model, discard_ratio=0.8, device=device
            )
        else:
            self.rollout = saliency_fn
...
def main():
    ...
    sampler = ProgrammableSampler("vit_base_patch16_224", dropout_rate=0.5)

    image_path = IMAGENET_VAL_DIR+"/ILSVRC2012_val_00050000.JPEG"
